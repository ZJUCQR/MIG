...
class VBench2(object):
    ...
    def build_full_dimension_list(self, ):
        return ["Dynamic_Attribute", "Dynamic_Spatial_Relationship", "Motion_Order_Understanding", "Complex_Plot"]

    def check_dimension_requires_extra_info(self, dimension_list):
        dim_custom_not_supported = set(dimension_list) & set([
            'Dynamic_Attribute', 'Dynamic_Spatial_Relationship', 'Complex_Plot', 'Motion_Order_Understanding'
        ])
    ...
    def build_full_info_json(self, videos_path, name, dimension_list, prompt_list=[], special_str='', verbose=False, mode='vbench_standard', **kwargs):
        ...
        if mode=='temporal_sequence':
            temporal_prompt_file = kwargs.get('temporal_prompt_file')
            assert temporal_prompt_file is not None, 'temporal_prompt_file is required for temporal_sequence mode'
            temporal_items = load_json(temporal_prompt_file)
            for item in temporal_items:
                item_dimension = item.get('dimension')
                if item_dimension not in dimension_list:
                    continue
                item_id = item.get('id')
                if not item_id:
                    continue
                image_dir = os.path.join(videos_path, item_id)
                if not os.path.isdir(image_dir):
                    print(f'WARNING!!! Temporal result directory not found: {image_dir}')
                    raise FileNotFoundError(image_dir)
                cur_full_info_list.append({
                    'id': item_id,
                    'prompt_en': item.get('prompt') or item.get('source_prompt_en') or '',
                    'dimension': [item_dimension],
                    'auxiliary_info': item.get('auxiliary_info', []),
                    'video_list': [image_dir],
                })
